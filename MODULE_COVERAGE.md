...
| M6 — Appointment Management | Core / Implemented frontend | Patient Book (Smart Find / Manual booking, both automatically Dentist-assigned) and Staff worklist; book, reschedule, cancel with canonical context; Patient Booking Drafts save in-progress choices without reserving anything | No payment during booking; Patient cannot be reassigned by rescheduling; admitted reschedule blocked; a Booking Draft is never an appointment |
| M7 — Scheduling / Conflict Prevention | Core / Implemented frontend | Deterministic automatic Dentist assignment (`assignDentist`) and open-time search (`findOpenTimes`) share the same branch/service/Dentist/shift/time/overlap validation as manual selection | Assignment/search reserve nothing until validated booking; no invented timing policy; no Patient Dentist free-choice |
