> **Status: Phase 4B (Patient experience, including M24 Referral & Loyalty) is complete.** This record is chronological: 4B.1, the 4B.1 polish pass, 4B.2, the 4B.2 acceptance pass and the final Phase 4B audit. Statements such as "4B.2 is not started" describe the state at the time they were written.
...
# Final Phase 4B audit

Audited `fa96cdd..6a81438` (Phase 4B.1 `7b27d00` and Phase 4B.2 `6a81438`) as one Patient product. No application code, test, business rule, ERD, dependency or backend change resulted; the findings were documentation drift, corrected below.

## Documentation corrections made by the audit

- Current guidance still described a Patient "Find Best" control. It existed only in the old Patient booking wizard and was replaced in 4B.1 by neutral "Suggested times" (Staff booking never had it). `README`, `MODULE_COVERAGE`, `FRONTEND_SCOPE`, `DOCUMENTATION_RECONCILIATION` and the demo guide now say "Suggested times".
- Retired Patient names ("My Appointments", "Your care", "Queue & Wait") and stale status lines (`dac864e`/"260 tests"/"Phase 4 pending") in `README`, `MODULE_COVERAGE`, `FRONTEND_SCOPE` and `PROFESSOR_DEMO_GUIDE` now match the shipped UI and the current 402-test / 13-scenario baseline.
- The demo guide gained the Journey Hub, the Referral & Loyalty demonstration steps (Owner enrolls through the first qualified activity) and an explicit list of Patient demo limitations.
- `AGENTS.md`, `CLAUDE.md` and the UI skill now name the same required Phase 4A/4B records. This record now opens with a completion banner.

## What was checked

- **Privacy:** a sweep with a complete Maria journey and a separate John record ran every Patient selector as John, two forged sessions, two deactivated accounts, an inactive session flag and no session (161 probes, 25 markers): no leak, every non-owner variant failed closed, and 9 cross-Patient commands were all rejected with state unchanged. Draft prescription and Draft invoice are withheld from their own owner; private complaint/plan/notes never reach the Patient.
- **Workflow:** Patient pages call only shared commands (`saveAppointment`, `cancelAppointment`, `provideHmoRequirement`, `markConversationRead`, `replyToConversation`, `requestLoyaltyRedemption`) and no setter; the Check-In and Queue commands found in `PatientFlow.jsx` belong to the Staff pages, and a Patient session is rejected by all three with no queue entry created.
- **M24 isolation:** the full book → check-in → treatment → prescription → invoice → payment chain succeeds, and the Patient sees the right records, with no loyalty account, a seeded account, an inconsistent ledger, a non-array collection and duplicate accounts.
- **Policy, branding, code quality:** no invented cutoff, arrival window, penalty, amendment or reopening rule in source or current docs; no visible product-name regression, logos byte-identical to `fa96cdd`, storage namespace, package name and dependencies unchanged; no `ROLE_INFO` Patient identity, raw Patient setter, `Date.now` domain logic, native dialog, artificial timer or console logging in the new code. Findings not fixed (pre-existing or cosmetic): three unused imports in `Admin.jsx` and four unreferenced exports in `components.jsx` that were already unused at `fa96cdd`; the identical "couldn’t confirm your account" notice is repeated in five Patient files.
- **Browser (headless Chrome, states rebuilt for the audit date):** 234-view five-width audit, 135-view matrix, 93 Patient journeys, 28 accessibility checks, focus-not-obscured (170 stops × 4 viewports), 200% zoom (70 views), 12 M24 journeys (36 checks), M24 matrix (35 views), 34 enrollment and request-cycle checks, reduced motion and live-region checks, and a connected-journey pass (one `h1` per page and no placeholder text or raw IDs across 8 states × 10 pages), all with 0 console errors. Cross-role pixel comparison against `fa96cdd` at 1440/1024/768/375: identical except the intentionally changed Owner Engagement page and the assistant panel captured over it; the time-driven HMO pages matched back-to-back.
- **Tests:** `npm test` 402/402, `npm run test:smoke` 13/13, `npm run build` passes with the >500 kB warning still visible.

## Remaining known limitations

Headless Chrome only (no screen reader, physical device, other browser or forced-colors testing); not a WCAG certification; the demo login reaches only Maria; nav labels, page titles and Home tiles differ slightly in casing and wording ("Prescriptions" vs "My Prescriptions", "Receipts & payments" vs "Receipts & Payments"), which was left as a style choice; a corrupt saved loyalty collection triggers the workspace-wide Saved Workspace Recovery gate exactly as every other corrupt collection does. P1–P9 remain `POLICY DECISION REQUIRED`.
