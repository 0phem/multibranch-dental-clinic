...
| Scheduling | Patient Book offers Smart Find (deterministic recommended options) or Manual booking (Branch → Service → Date → Time); both automatically assign the Dentist through the shared validator (current catalog, hours, shift, capability, Patient/Dentist overlap, future time); an in-progress choice may be saved as a Booking Draft, which reserves nothing | No booking payment; no invented cancellation/reschedule cutoff; no Patient Dentist free-choice |
...
| Referral & Loyalty (M24) | Approved Frontend Enhancement, implemented prototype: Patient sees their own referral code, ledger-validated points, activity history and can request a reward; authorized Staff/Owner record qualified activity and process requests, all through shared commands with session identity, replay protection and ledger validation | Team-designed prototype program rules (50-point threshold, one Pending request at a time, each request processed once, same-day duplicate-reward check) and validation rule (whole positive points), not historical clinic policy or an established clinic program. Staff/Owner enroll a Patient by recording their first qualified activity. No reward benefit, monetary value, tier, expiry or referral relationship is modeled; a normalized referral lifecycle is a later M24 extension. M25 has only a limited campaign-draft preview (no real campaigns; raw setter is known technical debt); full M25 is deferred |
