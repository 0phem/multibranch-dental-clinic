...
export function PatientsPage({ role, store }) {
  ...
  const createPatient=()=>{if(role!=='staff')return;if(!newPatient.name||!newPatient.phone)return toast('Patient name and phone are required.','warning');const duplicate=state.patients.find(p=>p.name.toLowerCase()===newPatient.name.toLowerCase()||p.phone===newPatient.phone);if(duplicate)return toast(`Possible duplicate record: ${duplicate.name}. Review the existing patient first.`,'warning');const record={id:uid('p'),...newPatient};setters.setPatients(xs=>[...xs,record]);log(ROLE_INFO.staff.name,`Created patient record ${record.name}`,'M4');toast('New centralized patient record created.','success');setSelected(record.id);setNewOpen(false)}
